...
export function TransactionTable({className = ""}: TransactionTableProps) {
  ...
  const fetchTransactions = async () => {
    try {
      ...
      const formattedTransactions = data.items.map((item: TransactionItem) => ({
        ...
        type:
          Array.isArray(item.transaction_types) &&
          item.transaction_types.length > 0
  ...
  return (
    <div
      className={`bg-black/80 backdrop-blur-md rounded-2xl shadow-2xl overflow-hidden border border-gray-800/50 w-full flex flex-col ${className}`}
    >
